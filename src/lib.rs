...
pub type Senders = HashMap<u64, mpsc::UnboundedSender<Bot>>;
    ...
    C: ConnectionTrait + TransactionTrait,
{
        ...
        .map_err(|err| error!("Portal retrieve error: {err}"))?
    {
        ...
        debug!("Can't find portal on coordinates {lat},{lon} on database, fallback to requesting");
    }
    ...
    C: ConnectionTrait + TransactionTrait,
{
    while let Some((poc, time)) = portal_rx.recv().await {
        ...
        if let Ok(portal_revision) = database::portal::Entity::get_latest_revision(conn, &id)
            .await
            .map_err(|err| error!("Portal {id} retrieve error: {err}"))
        {
            // scan portal only if informations are outdated
            if portal_revision.is_none() || portal_revision.as_ref().is_some_and(|revision| revision.timestamp < time) {
                if let Ok(res) = get_portal_details_with_retry(intel, &id)
                    .await
                    .map_err(|err| error!("Portal {id} scan error: {err}"))
                    && let Err(err) = database::portal::update_or_insert(
                        conn,
                        &id,
                        Utc::now().timestamp_millis(),
                        portal_revision,
...
pub async fn portal_survey(config: &config::Config, intel: &Intel<'_>, senders: &Senders) {
                ...
                .await
            {
                ...
                while let Some(portals) = stream.next().await {
                    for portal in portals.into_iter().flat_map(|p| p.entities) {
                        ...
                        if database::portal::should_scan(conn, &portal).await
                            && let Err(err) = portal_tx
                                .send((PortalOrCoords::Portal { id: portal.id }, Utc::now().timestamp_millis()))
                        {
                            error!("Portal scan enqueue error: {err}");
                        }
                ...
                if !list.is_empty() {
                    ...
                    for ((level, faction), sublist) in list {
                        let anchored_lists = split_by_anchor(sublist, &zone.anchors);

                        // split messages to respect 4094 bytes message limit
                        let msgs = anchored_lists.into_iter().fold(vec![], |mut msgs, (anchor, sublist)| {
                            let init = format!(
                                "{}{}{} L{level} portal:",
                                anchor.unwrap_or_default(),
                                if anchor.is_some() { " " } else { "" },
                                entities::Team::from(faction)
                            );
                            msgs.extend(sublist.into_iter().fold(vec![init.clone()], |mut msgs, portal| {
    ...
    T: Ord,
{
    ...
    if let Some(anchors) = anchors {
        ...
        for portal in sublist {
            ...
            let entry: &mut Vec<_> = temp.entry(nearest_anchor.map(|(name, _)| name)).or_default();
            entry.push(portal);
        }
        temp
    } else {
        let mut temp = HashMap::with_capacity(1);
        temp.insert(None, sublist);
        temp
    }
}
...
"#;
        ...
        assert!(split[&Some("Padova")].iter().any(|portal| portal.name == "Chiesa Cristo Risorto"));
        assert!(split[&Some("Padova")].iter().any(|portal| portal.name == "Cimitero Monticelli"));
        assert!(split[&Some("Bassano")].iter().any(|portal| portal.name == "Barbeque di comunità"));
        assert!(split[&Some("Bassano")].iter().any(|portal| portal.name == "Capitello Di San Antonio"));
        assert!(
            split[&Some("Bassano")]
                .iter()
                .any(|portal| portal.name == "Capitello Madonna Di Lourdes Bassano del Grappa")
        );
        assert!(split[&Some("Bassano")].iter().any(|portal| portal.name == "Chiesa New Cambridge Institute"));
        assert!(split[&Some("Bassano")].iter().any(|portal| portal.name == "Gufo in legno di Via Toscanini"));
        assert!(split[&Some("Bassano")].iter().any(|portal| portal.name == "Madonna"));
        assert!(split[&Some("Bassano")].iter().any(|portal| portal.name == "Piccolo Capitello Alla Madonna - Rosà"));
        assert!(split[&Some("Jesolo")].iter().any(|portal| portal.name == "Die Musikanten"));
    }
    ...
    fn plexts() {
        ...
        let expected = [
            DestroyedLink {
                player: Player { team: Resistance, name: "CeccoMan" },
                source: Portal {
            ...
            DestroyedReso {
                player: Player { team: Resistance, name: "btgalpi" },
                portal: Portal {
            ...
            CreatedCF {
                player: Player { team: Resistance, name: "Z0FK4" },
                portal: Portal {
            ...
            Captured {
                player: Player { team: Enlightened, name: "guerrafix" },
                portal: Portal {
                    name: "La tradotta. 12",
